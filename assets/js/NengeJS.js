...
window['NengeNet'] = new class NengeApp {
    onReady(){
        this.DB = this.localForage.createInstance({'name': 'NengeNet','storeName': "IndexPage"});
        this.DB.keys().then(db_list => {
            this.log('NengeNet DB ready');
            this.log(db_list);
            this.DB_LOAD('base.zip');
            this.DBcomplete();
        });
    }
    DBcomplete(){
    ...
    constructor() {
        let N = this,
            ...
            DOMC = 'DOMContentLoaded',
            onReady = ()=>{
                this.assetsDIR = currentScript.src.split('/').slice(0, -2).join('/') + '/';
                N.E(dom,'remove',DOMC,onReady, false);N.onReady(); 
            };
        N.E(dom,'add',DOMC, onReady, false);
        this.log('NengeNet class onload.');
    }
    DB_LOAD(DB_ID,NoCache=false) {
        let tableData = this.DB_LIST[DB_ID] || {'ver':this.ver},
            ver = tableData.ver,
            fileType = DB_ID.split('.')[1];
        if (!DB_ID) return;
        if (this.localServer){ 
            return tableData.files ? tableData.files.forEach(file => {this.LINK_ADD(file);}):this.LINK_ADD(DB_ID);
        }
        let fetchDate = ()=>{
            if(fileType == 'zip'&&!this.JSZip){
            ...
            this._Fetch('js/'+DB_ID+'?'+Math.random()).then(data => {
                if(fileType == 'zip'){
                        ...
                        let files = {}, dataLength=0,readLength=0,
                            check = (fileName, text) => {
                                ...
                                if (dataLength==readLength) {
                                    this.DB.setItem(DB_ID, {files,ver});
                                    for (var i in files) {
                                        this.LINK_SET(i,files[i],DB_ID);
                                    }
                                }
                            },
                            getFile = (fileName) => {
                                ZipData.file(fileName).async("arraybuffer").then(FileData => check(fileName,FileData));
                            };
                        for (var fileName in ZipData.files) {
                            if(ZipData.files[fileName].dir == true) continue;
                            dataLength++;
                            getFile(fileName);
                    ...
                    this.DB.setItem(DB_ID, {data,ver});
                    this.LINK_ADD(DB_ID,data);
                }
            });
        }
        if(NoCache)fetchDate();
        this.DB.getItem(DB_ID).then(data => {
            if (!data || data.ver != ver) {
                fetchDate();
            } else {
                if(data.files){
                    for (var i in data.files) {
                        this.LINK_SET(i,data.files[i],DB_ID);
                    }
                }else if(data.data){
                    this.LINK_ADD(DB_ID, data.data);
                }
    ...
    DB_ZIP(){
       this.DB_ZipFiles.forEach(zip=>this.DB_LOAD(zip.DB_ID,zip.NoCache));
       this.DB_ZipFiles=[];
    }
    LINK_TYPE(type) {
        switch (type) {
    ...
    LINK_SET(file,data,DB_ID){
        let type = this.LINK_TYPE(file.toLowerCase().split('.').pop());
        if(type){
            this.LINK_ADD(file, data);
        }else {
            if(!this.FILE[DB_ID])this.FILE[DB_ID] = {};
            this.FILE[DB_ID][file] = window.URL.createObjectURL(new Blob([data]))
        };
    }
    LINK_ADD(name, data) {
        ...
        if(_t[4]&&_t[4]=='fonts'){
            ...
            return this.LINK_ADD(filename+'.css',str);
        }
        this.FILE[name] = link;
    ...
    _Fetch(file, type) {
        return this.Fetch(this.assetsDIR+file, type);
    }
    async Fetch(file, type) {
        console.log(file);
        let f = await fetch(file);
        return await f[type||'arrayBuffer']();
    }
    CustomElemtData = {};
    createElement(myelement, func=()=>{},func2=()=>{}) {
        ...
        class MyElement extends HTMLElement {
            ...
            disconnectedCallback() {
                this.CLOSEFUNC('remove', this);
            }
        }
        window.customElements.define(myelement, MyElement);
    }
    E(dom,action,event,func,pass=false){
        let e = 'EventListener';
        this.log(Array.of(event,action,e));
        return dom[action+e](event,func,pass);
    }
    log(arr,color,type="log"){
        let arrs = typeof arr =='string' ? [arr]:arr,
            str = ['%c '+arrs.join(' %c ')],
            c = (d,e)=>'color:'+d+';background:'+e+';',
            colors = color || [c('#fff','#000'),c('#fff','blue'),c('#fff','red')];
            for(var i in arrs){
                str.push(colors[i%colors.length]);
            }
            console[type].apply(null,str);
    }
